...
func (_q *AgentQuery) QueryMemberships() *GroupMembershipQuery {
...
func (_q *AgentQuery) QueryPolicyBindings() *PolicyBindingQuery {
...
func (_q *AgentQuery) WithMemberships(opts ...func(*GroupMembershipQuery)) *AgentQuery {
	query := (&GroupMembershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AgentQuery) WithPolicyBindings(opts ...func(*PolicyBindingQuery)) *AgentQuery {
	query := (&PolicyBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AgentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Agent, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *Agent) { n.Edges.Memberships = []*GroupMembership{} },
			func(n *Agent, e *GroupMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AgentQuery) loadMemberships(ctx context.Context, query *GroupMembershipQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *GroupMembership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupmembership.FieldAgentID)
	}
	query.Where(predicate.GroupMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.MembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *AgentQuery) loadPolicyBindings(ctx context.Context, query *PolicyBindingQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *PolicyBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Agent)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(policybinding.FieldAgentID)
	}
	query.Where(predicate.PolicyBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.PolicyBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
